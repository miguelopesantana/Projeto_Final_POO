...
public class Loja implements Serializable {
    ...
    private void menu(File f, Loja loja, boolean frequente, Data data) {
        try {
            int escolha;
            ...
            switch (escolha) {
                case 1 -> loja.login(0);
                case 2 -> data.setData();
                case 3 -> loja.comprar(f, frequente, data);
                case 4 -> loja.listaRecibos();
                case 0 -> loja.save(f,1);
            }
    ...
    private boolean lerFicObj(File f, int n) {
        try {
     ...
     *
     * @param k indica se é para adicionar as informações lidas, ou apenas devolver um booelan
     * @return true se todos os ficheiros forem lidos com sucesso, false caso ocorra algum erro
     */

    private boolean lerFicstxt(int k) {
...
//        return true;
     ...
     * @param k inteiro que indica se é para adicionar os clientes lidos ao ficheiro ou não
     * @return true se o ficheiro for lido com sucesso, false caso ocorra algum erro
     */

    private boolean lerClientes(File f, int n, int k) {
     ...
     *
     * @param f ficheiro de produtos
     * @param k inteiro que indica se é para adicionar os produtos lidos ao ficheiro ou não
     * @return true se o ficheiro for lido com sucesso, false caso ocorra algum erro
     */

    private boolean lerProdutos(File f, int k) {
        if (f.exists() && f.isFile()) {
            try {
                FileReader fr = new FileReader(f);
                BufferedReader br = new BufferedReader(fr);
    ...
    private boolean verificar(String email, List<Cliente> lista, int n) {
        for (Cliente c : lista) {
     ...
     *
     * @param f         ficheiro de objetos para o qual se pretende guardar as transações efetuadas
    ...
    private void comprar(File f, boolean frequente, Data data) {
...
//                    System.out.println(produtosDisponiveis.get(i).preco * (1 - (0.05 * p)));
                ...
                if (produtosDisponiveis.size() == 0) {
                    ...
                    System.out.println("--------------------------------------------------------\n");
                    switch (escolha) {
                        case 1 -> comprar(f, frequente, data);
                        case 0 -> recibo(f, frequente, preco);
                    }
                }
            }
        }
    ...
    private boolean verificaPromocao(Data data, Promocao promocao) {
    ...
    private void recibo(File f, boolean frequente, float preco) {
        int precot = 0;
    ...
    private void listaprodutosDisponiveis() {
        int k = 0;
...
//                r.tostring();
    ...
    private void save(File f, int n) {
        try {
